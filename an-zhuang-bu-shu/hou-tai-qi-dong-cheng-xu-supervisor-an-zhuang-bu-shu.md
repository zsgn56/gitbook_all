...
;directory=/var/supervisor              ; (default is not to cd during start)
;nocleanup=true              ; (don't clean up tempfiles at start;default false)
;childlogdir=/var/supervisor            ; ('AUTO' child log dir, default $TEMP)
...
;directory=/var/supervisor                ; directory to cwd to before exec (def no cwd)
...
environment=JAVA_HOME="/usr/local/jdk1.8.0_131",JAVA_BIN="/usr/local/jdk1.8.0_131/bin",ES_HOME=/usr/local/elasticsearch
user=elasticsearch
minfds=65536
...
```
supervisor启动时并不会加载/etc/profile文件，需配置参数enviroment
...
```
运行某个程序时提示 max file descriptors [4096] for elasticsearch process is too low, increase to at least [65536]
